...
const MyProducts = (props) => {
    ...
    const hadndler = async(e,ProductId)=>{
      ...
      try{
         const transaction = await props.contract.Resale(ProductId,updatedPrice,UpdatedERTNPrice)
    ...
    const deleteTweet = async(e,tweet)=>{
      e.preventDefault()
      try{
        const transaction = await props.contract.DeletingText(props.account,tweet);
        await transaction.wait();
        alert("Tweet Deleted");
      }catch(e){
        console.log(e)
  ...
  return(props.trigger)?(
    ...
    <center><br/>
      <span>{Userinfo[0]} {Userinfo[1]}</span><br/><br/>
      <img src={Userinfo[2]} class="profileclass" height="100px" width="100px"/><br/><br/>
      <span>{Userinfo[3]}</span><br/><br/>
    </center><br/>

    <h5>YOUR TOKENS : {ERTN} ERTNS</h5><br/>
    {tweetStatus == true?<h5>YOUR TWEETS </h5>:""}
    <center>
      {
        data.map((memo)=>memo.owner== props.account?
          <div>
            <div class="textblock">
                   <p>{new Date(memo.timestamp * 1000).toLocaleString()}&nbsp;<img src="https://cdn-icons-png.flaticon.com/512/3096/3096687.png" onClick={(e)=>{deleteTweet(e,count++)}} width="20px" height="20px"/></p>
                   {memo.NFTUrl=='none'?<p class="msg">{memo.text}</p>:<div><p>Hello Everyone I have shared NFT checkout</p><img src={memo.NFTUrl} title='Delete Tweet' width="100px" height="100px"/> <p>{memo.text}</p></div>}

             </div>
             <br/>
          </div>:<p></p>)}
    </center><br/>
    {Object.keys(product).length == 0 ?<span></span>:<center><h5>YOUR NFTS</h5></center>}
    <div className="inner">
           {Object.entries(product).map(([ProductID,[nftname,imageUrl,price,status,priceERTN]], index) => (
            <div className="Productitem" key={index}>
              <div className="productbody">
                <center>
                  <img src={imageUrl} width="260px" height="200px" alt="Product" /><br/><br/>
                  <p>{nftname}</p>
                  <p><b>Price: {price} Wei or {priceERTN}ERTN</b></p>
