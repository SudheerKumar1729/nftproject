  ...
  import Feed from './Feed';
    const Home = (props) => {
      ...
      const handleSearchChange = (event) => {
        ...
        filterProducts(); // Filter products when search query changes
      };
      return (
        <div>
          ...
          <div class="NFTHeader">
           ...
           {Object.keys(product).length == 0 ?<center><h5>Currently No Live NFTS</h5></center>:""}  
            <div className="inner">
               {Object.entries(searchQuery ? filteredProducts : product).map(([ProductID, [nftname, imageUrl, price, status, seller, priceERTN]], index) => (
                status == false ? "" :
                <div className="Productitem" key={index}>
                  <div className="productbody">
                    <center>
                      ...
                      <button onClick={() => {handleBuyClick(nftname,price,imageUrl,ProductID,priceERTN);alert("Added to Cart")}} class="btn btn-dark">BUY</button>
                    </center>
                  </div>
                </div>
              ))}
